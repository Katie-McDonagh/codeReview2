describe ("soundWaveFilter", function (){
  ...
  xit ("returns a single soundwave modified when the lower limit is breached", function (){
    expect(soundWaveFilter([9], 10, 30)).toEqual([10])
  })

  xit ("returns a single soundWave modified when the upper limit is reached", function (){
    expect(soundWaveFilter([31], 10, 30)).toEqual([30])
  })

  xit ("returns multiple soundwaves unchanged when the upper and lower limits are not reached", function (){
